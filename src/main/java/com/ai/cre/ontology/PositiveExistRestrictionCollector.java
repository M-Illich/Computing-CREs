 ...
 * positively on the left-hand side of axioms. Counterpart to
 * {@link NegativeExistRestrictionCollector}.
 *
 */
public class PositiveExistRestrictionCollector implements OWLClassExpressionVisitorEx<Set<OWLObjectSomeValuesFrom>> {

	@Override
	public Set<OWLObjectSomeValuesFrom> visit(OWLObjectComplementOf ce) {
		// switch visitor for negated concept
		return ce.getOperand().accept(new NegativeExistRestrictionCollector());
	}
	...
	@Override
	public Set<OWLObjectSomeValuesFrom> visit(OWLObjectSomeValuesFrom ce) {
		Set<OWLObjectSomeValuesFrom> rest_set = new HashSet<>();
		rest_set.add(ce);
